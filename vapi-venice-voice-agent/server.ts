...
// The "compute" plane. Vapi's custom-llm calls Venice through Floe's METERED
// endpoint (pooled float pays Venice; the agent is billed at-cost token usage)
// — so inference draws on the SAME session spend-limit as the Exa tool below.
// Model + tools, one ceiling. Model list: /vendors/venice-compute.
...
// LLM turns get a longer fuse than tool calls: a transient Venice latency spike
// must not abort mid-voice (that's what aborted the first recording at 15s).
...
// Compute plane: Vapi's custom-llm posts here; we pay Venice through Floe so
// inference draws down the same session budget the taper/hard-stop enforce.
...
app.listen({ port: PORT, host: "0.0.0.0" }, (err) => {
  ...
  console.log(`   LLM endpoint:       POST /llm/chat/completions → Venice (${VENICE_MODEL}) via Floe`);
  ...
  console.log(`   One credit line caps BOTH the model and the tools.`);

...
| **vapi-custom-llm** | TypeScript | Vapi (custom LLM) | Intermediate | Point a Vapi assistant's custom LLM at Floe's keyless gateway — the model leg (~60% of a typical call's cost) is metered per token and refused `402` pre-call at the cap; an optional shim turns the refusal into a spoken goodbye. | [→](./vapi-custom-llm) |
| **retell-custom-llm** | TypeScript | Retell (custom LLM WebSocket) | Intermediate | The WebSocket adapter Retell's custom LLM actually needs: a ~150-line server that turns each Retell turn into a streaming Floe gateway call — metered, pre-call gated, speaking "I've reached my budget limit" and hanging up cleanly on `402`. | [→](./retell-custom-llm) |
| **vapi-voice-metered** | TypeScript | Vapi (custom transcriber + voice) · Deepgram · ElevenLabs | Advanced | Put Floe **inside** Vapi's provider slots: STT and TTS run through Floe's orchestrator surfaces, metered per audio-second / per character — combined with `vapi-custom-llm`, every leg except telephony is pre-call gated on one ledger. | [→](./vapi-voice-metered) |
| **livekit-voice-agent** | Python | LiveKit Agents · Deepgram | Intermediate | A real-time LiveKit voice agent whose LLM and TTS legs meter on one Floe key via a `base_url` swap — STT ships BYO Deepgram, with Floe's streaming STT WebSocket as the drop-in upgrade. | [→](./livekit-voice-agent) |
